...
bool Scene::LoadTestGeometry()  {
     ...
     */
    RTCGeometry geom = rtcNewGeometry(Bounce::globalDevice,
    ...
    if (vertices && indices)
    {
        vertices[0] = 0.f; vertices[1] = 0.f; vertices[2] = 5.f;
        vertices[3] = 1.f; vertices[4] = 0.f; vertices[5] = 5.f;
        vertices[6] = 0.f; vertices[7] = 1.f; vertices[8] = 5.f;
...
void recurse(const IObject &object, Scene &scene) {
    ...
    if (IPolyMeshSchema::matches(props.getHeader())) {
        ...
        printf("#     Loading %s : %zu points...\n", object.getName().c_str(), positions->size());
...
//        auto *rtcNormals = (unsigned *) rtcSetNewGeometryBuffer(geom,
